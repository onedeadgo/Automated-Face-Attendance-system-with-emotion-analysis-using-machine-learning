...
count=0
if not os.path.exists('./resizedTrainingImages'):
  os.makedirs('./resizedTrainingImages')
...
for path, subdirnames, filenames in os.walk("dataset"):
  for filename in filenames:
    ...
    if not os.path.exists(f'./resizedTrainingImages/{str(id)}'):
      os.makedirs(f'./resizedTrainingImages/{str(id)}')
